...
function authUser(req, res, next) {
  ...
  if (url === '/users/authenticate' || (url === '/users' && req.method === 'POST')) {
    next()
  } else {
    ...
    if (bearerToken) {
      ...
      try {
        ...
        req.curuser = payload
        req.curuser.role = payload.email == 'admin@gmail.com' ? 'ROLE_ADMIN' : 'ROLE_USER'
        // authorization
        // allow /foods/* POST, PUT, DELETE only to ADMIN
        if (url.startsWith('/foods') && req.method !== 'GET' && req.curuser.role !== 'ROLE_ADMIN')
